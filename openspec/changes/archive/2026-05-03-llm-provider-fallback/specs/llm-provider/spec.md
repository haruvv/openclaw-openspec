...
システムは、テキスト生成リクエストを `gemini-1.5-flash` モデルに送信しなければならない（SHALL）。
...
システムは、Gemini API が HTTP 429 を返した場合に限り、Z.ai の Anthropic互換エンドポイント経由で GLM モデルを呼び出しフォールバックしなければならない（SHALL）。
