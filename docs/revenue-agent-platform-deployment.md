...
初期設定では、コンテナを常時起動にせず、短めの idle timeout で sleep させる方針にします。`wrangler.jsonc` の `REVENUE_AGENT_CONTAINER_SLEEP_AFTER` は `1m` にし、Container class 側でも idle timeout 到達時に明示的に `stop()` を呼びます。
...
Container は低頻度運用時の固定費を抑えるため `instance_type=basic`、`max_instances=1` にしています。`basic` は `standard-1` より memory/disk の provision 量が小さいため、起動中の Memory GiB-second と Disk GB-second を抑えられます。deploy / rollout 直後の一時的な余裕よりも、想定外に複数 instance が起動したままになるリスクを下げる方を優先します。高可用性、同時実行、Lighthouse/Chromium の OOM が必要になった場合だけ `standard-1` 以上へ戻します。
