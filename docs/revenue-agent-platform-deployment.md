...
初期設定では、コンテナを常時起動にせず、短めの idle timeout で sleep させる方針にします。`wrangler.jsonc` の `REVENUE_AGENT_CONTAINER_SLEEP_AFTER` は `3m` にし、`LIGHTHOUSE_TIMEOUT_MS=90000` より十分長くします。Container class 側でも idle timeout 到達時に明示的に `stop()` を呼びます。

Lighthouse/Chromium は無限実行させず、Node の `execFile` timeout で子プロセスごと hard kill します。Lighthouse が timeout した URL は crawl-only fallback scoring で処理を継続し、automation 全体を止めない方針です。
...
Container は低頻度運用時の固定費を抑えるため `instance_type=basic` にしています。`basic` は `standard-1` より memory/disk の provision 量が小さいため、起動中の Memory GiB-second と Disk GB-second を抑えられます。`max_instances=2` は deploy / rollout 直後に旧 instance と新 instance が一時的に重なる余地を残すための上限で、常時 2 台を起動する意図ではありません。通常運用は Worker version ごとの singleton 名と `3m` sleep に寄せます。高可用性、同時実行、Lighthouse/Chromium の OOM が必要になった場合だけ `standard-1` 以上へ戻します。

現在の Container instance 状態は次で確認します。
...
Container を即時停止したい場合は、`REVENUE_AGENT_INTEGRATION_TOKEN` を bearer token として内部停止 endpoint を呼びます。

```bash
curl -X POST "https://<production-hostname>/internal/container/stop" \
  -H "Authorization: Bearer <REVENUE_AGENT_INTEGRATION_TOKEN>"
```

この endpoint は Worker 側で token を検証してから現在の named Container instance に `stop()` を送ります。停止後の次回リクエストでは cold start が発生します。
